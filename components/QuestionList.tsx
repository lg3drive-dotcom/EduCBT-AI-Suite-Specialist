          ...
          <div key={q.id} className="space-y-4">
            {showStimulus && (
              <div className="bg-slate-100 border-l-4 border-indigo-500 p-6 rounded-r-xl shadow-sm space-y-4">
                ...
                <MathText text={q.stimulusText!} className="text-slate-700 text-sm leading-relaxed whitespace-pre-wrap font-medium bg-white/50 p-4 rounded-lg border border-slate-200/50 italic" />
              </div>
            )}
            ...
            <div className={`bg-white rounded-xl border shadow-sm overflow-hidden transition-all ${isTrashView ? 'opacity-70' : 'border-slate-200'}`}>
              <div className="bg-slate-50 px-4 py-2 border-b flex justify-between items-center">
                <div className="flex items-center gap-3">
                  <div className="flex items-center justify-center w-8 h-8 bg-indigo-600 text-white rounded-lg text-xs font-black shadow-inner">{q.order}</div>
                  <span className="text-[10px] font-black px-2 py-0.5 bg-indigo-100 text-indigo-700 rounded uppercase">{q.level}</span>
                  <span className="text-[10px] font-bold text-slate-400 uppercase tracking-tighter">{q.type}</span>
                </div>
                <div className="flex gap-1">
                   ...
                   <button onClick={() => onDelete(q.id)} className="p-1.5 text-rose-500 hover:bg-rose-50 rounded"><svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7" /></svg></button>
                </div>
              </div>

              <div className="p-5 space-y-4">
                <MathText text={q.text} className="text-slate-800 font-bold whitespace-pre-wrap leading-relaxed" />
                    ...
                    <table className="w-full text-sm">
                      <thead className="bg-slate-100 text-[10px] font-black uppercase text-slate-600">
                        <tr>
                          ...
                          <th className="px-4 py-3 text-left">Pernyataan</th>
                          <th className="px-4 py-3 text-center w-24">{trueLabel}</th>
                          <th className="px-4 py-3 text-center w-24">{falseLabel}</th>
                        </tr>
                      </thead>
                      <tbody>
                        {q.options.map((opt, i) => {
                          ...
                          return (
                            <tr key={i} className="border-t hover:bg-slate-50">
                              ...
                              <td className="px-4 py-3"><MathText text={opt} className="text-slate-700 font-medium" /></td>
                              <td className="px-4 py-3 text-center">
                                <div className={`w-6 h-6 mx-auto rounded-lg border-2 flex items-center justify-center ${isTrue ? 'bg-emerald-500 border-emerald-500' : 'border-slate-200 bg-white'}`}>
                              ...
                              <td className="px-4 py-3 text-center">
                                <div className={`w-6 h-6 mx-auto rounded-lg border-2 flex items-center justify-center ${!isTrue ? 'bg-rose-500 border-rose-500' : 'border-slate-200 bg-white'}`}>
                                  {!isTrue && <svg className="w-4 h-4 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="4" d="M5 13l4 4L19 7" /></svg>}
                                </div>
                              </td>
                            </tr>
                    ...
                    {q.options.map((opt, i) => {
                      ...
                      return (
                        <div key={i} className={`p-3 rounded-xl border-2 flex items-center gap-4 transition-all ${isCorrect ? 'bg-emerald-50 border-emerald-300 shadow-sm' : 'bg-slate-50 border-slate-100'}`}>
                          ...
                          <MathText text={opt} className="text-sm font-semibold text-slate-700 flex-grow" />
                        </div>
                      );
                    })}
                  </div>
                ...
                {q.explanation && (
                  <div className="mt-4 p-4 bg-amber-50/50 rounded-xl text-[11px] text-amber-900 border border-amber-100 flex gap-3 items-start italic">
